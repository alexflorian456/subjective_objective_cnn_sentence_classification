...
vocab_file = "/home/alex/nlp/cnn-text-classification-tf/corola.300.20.vec"
...
def create_vocabulary_dict(vocab_file):
    vocab_dict = {}
...
def sentences_to_array(sentences, vocab_dict, max_document_length):
    sentence_array = np.zeros((len(sentences), max_document_length), dtype=np.int32)
    unfound_word_id = len(vocab_dict.keys()) + 1
    unfound_words_dict = {}
        ...
        for word_idx, word in enumerate(sentence.split(' ')):
            ...
            if word not in vocab_dict.keys():
                if word not in unfound_words_dict.keys():
                    ...
                    unfound_word_id += 1
                    word_array_value = unfound_word_id
                else:
                    word_array_value = unfound_words_dict[word]
            else:
                word_array_value = vocab_dict[word]
...
def preprocess():
    ...
    vocab_dict = create_vocabulary_dict(vocab_file)
    x = sentences_to_array(x_text, vocab_dict, max_document_length)
